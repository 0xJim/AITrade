...
def execute_open(symbol: str, direction: str, usd_amount: float, leverage: int,
                 sl_price: float = None) -> dict:
    """在币安真实下单开仓，成功后挂STOP_MARKET止损单"""
    result = {"success": False, "order": None, "quantity": 0, "error": "", "sl_order": None}
    ...
    try:
        ...
        if order and order.get("status") in ("NEW", "FILLED", "PARTIALLY_FILLED"):
            ...
            # 开仓成功后挂交易所级止损单 (STOP_MARKET reduceOnly)
            if sl_price and sl_price > 0:
                try:
                    sl_side = "SELL" if direction == "long" else "BUY"
                    sl_order = place_order(symbol, sl_side, quantity,
                                          order_type="STOP_MARKET",
                                          stop_price=sl_price,
                                          reduce_only=True)
                    if isinstance(sl_order, dict) and sl_order.get("status") in ("NEW",):
                        result["sl_order"] = sl_order
                        log(f"止损单已挂: STOP_MARKET {sl_side} qty={quantity} stop={sl_price}")
                    else:
                        log(f"止损单结果: {sl_order}")
    ...
    try:
        ...
        if order and order.get("status") in ("NEW", "FILLED", "PARTIALLY_FILLED"):
            ...
            try:
                ...
                log(f"取消{symbol}遗留挂单: {cancel_result}")
            except Exception as e:
                log(f"取消挂单异常: {e}")
        else:
...
def quick_scan(open_symbols, cooldowns):
    ...
    for t in tickers:
        ...
        if ENABLE_EXTREME_NEG_FUNDING and fr < EXTREME_NEG_FUNDING:
...
def deep_check(cand):
    ...
    elif cand["type"] == "spike":
        # spike信号: 已在spike_scanner中验证(15m异动+ATR)，只需快速确认K线有效
        # 不需要费率历史验证——spike是基于价格异动而非费率
        pass  # 直接进入技术指标确认
    ...
    # === 3. ATR动态止损 ===
    # spike信号使用自己的15m ATR止损（回测验证过的参数）
    if cand.get("type") == "spike" and cand.get("sl_pct"):
        atr_pct = cand.get("atr_pct", tech.get("atr_pct", 0))  # 保留spike的15m ATR
    else:
        atr_pct = tech.get("atr_pct", 0)
        if atr_pct > 0:
...
def check_time_stop(trade, now):
    """时间止损 — Spike信号专用: 持仓超MAX_HOLD_HOURS强制平仓"""
    try:
        from config import SPIKE_MAX_HOLD_HOURS
        if trade.get("signal_type") != "spike":
            return None, None  # 非spike不做时间止损
        ...
        try:
            ...
            if hours_held >= SPIKE_MAX_HOLD_HOURS:
                return "spike_max_hold", f"Spike持仓{hours_held:.1f}h≥{SPIKE_MAX_HOLD_HOURS}h"
        except Exception:
            pass
    except ImportError:
        pass
...
def main():
    ...
    def log_decision(symbol, action, reason, details=None):
        """记录每次扫描决策到JSONL"""
        entry = {
            "timestamp": now_str(),
            "symbol": symbol,
            "action": action,  # "open" / "reject" / "skip"
            "reason": reason,
        ...
        try:
            with open(SCAN_DECISIONS_FILE, "a") as f:
    ...
    for trade in open_positions:
        ...
        # v13: Spike持仓时间止损 (数据:>6h表现急剧下降)
        if not triggered:
            time_reason, time_detail = check_time_stop(trade, now)
            if time_reason:
                triggered = time_detail
    ...
    # === 信号源1: 传统信号(极端费率/暴跌暴涨/OI异动/费率翻转) ===
    candidates = quick_scan(open_symbols, state.get("last_opens", {}))

    # === 信号源2: 15分钟异动(spike) — 回测验证的主力盈利信号 ===
    spike_candidates = scan_spike_signals(
        open_symbols,
        state.get("last_opens", {}),
        state.get("spike_cooldowns", {})
    )
    if spike_candidates:
        # 去重: 跳过已被传统信号覆盖的币种
        ...
        log(f"  spike信号: +{len([s for s in spike_candidates if s['symbol'] not in existing_syms])}个新信号")
    ...
    # 按信号类型优先级排序: spike > 其他 > 极端费率
    # spike信号是回测验证的盈利信号，优先处理
    def signal_priority(c):
        sig_type = c.get("type", "")
        if sig_type == "spike":
            return (0, -abs(c.get("chg_pct_15m", 0)))  # spike优先，按异动幅度排序
        elif sig_type in ("crash_bounce", "pump_short"):
            return (1, -abs(c.get("change", 0)))  # 暴跌暴涨次之
        elif sig_type == "oi_surge":
            return (2, 0)  # OI异动
        else:
            return (3, -abs(c.get("fr", 0)))  # 极端费率/费率翻转最后
    ...
    for cand in candidates[:15]:  # v3: 放宽检查数量 10→15
        ...
        if V8_ENABLED:
            ...
            # v13: Spike信号使用更高的质量门槛 (65-70是死亡区间)
            try:
                from config import SPIKE_MIN_SIGNAL_QUALITY
                sq_threshold = SPIKE_MIN_SIGNAL_QUALITY if verified.get("type") == "spike" else V8_SIGNAL_QUALITY_MIN
            except ImportError:
                sq_threshold = V8_SIGNAL_QUALITY_MIN
            if signal_quality < sq_threshold:
                log(f"  v8信号质量{signal_quality:.0f}<{sq_threshold}{'(spike门槛)' if verified.get('type') == 'spike' else ''}，跳过")
                continue
            ...
            # ═══ v13: Spike高质量加仓 (质量≥80时×1.3) ═══
            try:
                from config import SPIKE_MIN_QUALITY_BOOST_MULT
                if verified.get("type") == "spike" and signal_quality >= 80:
                    original_pos = pos_usd
                    pos_usd = round(pos_usd * SPIKE_MIN_QUALITY_BOOST_MULT, 2)
                    log(f"  v13 Spike高质量加仓 ×{SPIKE_MIN_QUALITY_BOOST_MULT}: ${original_pos:.0f}→${pos_usd:.0f} (质量={signal_quality:.0f})")
            except ImportError:
                pass

            # ═══ v11j: 单笔亏损上限 — 开仓前缩仓(基于已知信息,不偷看未来) ═══
            # 缩仓逻辑: est_risk = position_usd × signal_sl_pct × leverage
            #            若 est_risk > MAX_LOSS_PER_TRADE，按比例缩小position_usd
            try:
        ...
        trade = {
            ...
            "direction": verified["direction"],
            "leverage": LEVERAGE,
            "position_pct": round(pos_usd / balance * 100, 2) if balance > 0 else POSITION_PCT,
        ...
        # 更新spike冷却
        if verified.get("type") == "spike":
            if "spike_cooldowns" not in state:
                state["spike_cooldowns"] = {}
            sym = verified["symbol"]
            direction = verified["direction"]
            if sym not in state["spike_cooldowns"]:
                state["spike_cooldowns"][sym] = {}
            state["spike_cooldowns"][sym][direction] = now_str()
        save_json(TRADES_FILE, data)
        ...
        log(f"开仓 #{trade['id']} {verified['symbol']} {d_cn} @ {trade['entry_price']} RR={rr:.1f} profile={STRATEGY_PROFILE}")
        log_decision(verified["symbol"], "open", f"#{trade['id']} {d_cn} RR={rr:.1f}", {
            "trade_id": trade["id"],
            "direction": verified["direction"],
