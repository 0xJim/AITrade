...
_LOCK_FILE = Path(__file__).resolve().parent.parent / "symbol_locks.json"

def acquire_symbol_lock(symbol: str, profile: str) -> bool:
    """尝试获取symbol锁，防止两个Profile同时开同一个币"""
    _lp = str(_LOCK_FILE) + ".lock"
    with open(_lp, "w") as _lf:
        _fcntl.flock(_lf, _fcntl.LOCK_EX)
        try:
            locks = load_json(_LOCK_FILE, {})
            holder = locks.get(symbol)
            if holder and holder != profile:
                return False
            locks[symbol] = profile
            save_json(_LOCK_FILE, locks)
            return True
        finally:
            _fcntl.flock(_lf, _fcntl.LOCK_UN)

def release_symbol_lock(symbol: str, profile: str):
    """释放symbol锁"""
    _lp = str(_LOCK_FILE) + ".lock"
    with open(_lp, "w") as _lf:
        _fcntl.flock(_lf, _fcntl.LOCK_EX)
        try:
            locks = load_json(_LOCK_FILE, {})
            if locks.get(symbol) == profile:
                locks.pop(symbol, None)
                save_json(_LOCK_FILE, locks)
        finally:
            _fcntl.flock(_lf, _fcntl.LOCK_UN)

def get_locked_symbols(exclude_profile: str = None) -> set:
    """获取所有被锁定的symbol（可排除自己的profile）"""
    locks = load_json(_LOCK_FILE, {})
    if exclude_profile:
        return {s for s, p in locks.items() if p != exclude_profile}
    return set(locks.keys())
            ...
            try:
                ...
                if klines_15m and len(klines_15m) >= 2:
                    ...
                    if open_15m > 0:
                        ...
                        # ★ v13过滤1: 只做多
                        if SPIKE_LONG_ONLY and direction == "short":
                            pass  # 跳过做空异动，继续扫描其他信号类型
                        
                        elif abs(move_pct) >= CLOSED_15M_ANOMALY_THRESHOLD_PCT:
                            # ★ v13过滤2: 1h EMA必须多头排列
                            if SPIKE_REQUIRE_EMA_UP and direction == "long":
                                ...
                                if klines_1h and len(klines_1h) >= 21:
                                    ...
                                    if not (ema9 and ema21 and ema9 > ema21):
                                        continue  # EMA不满足多头排列，跳过
                                else:
                                    continue  # 无1h数据，跳过
                            
                            # ★ v13过滤3: RSI门槛
                            rsi_15m = calc_rsi([float(k[4]) for k in klines_15m], 14)
                            if SPIKE_MIN_RSI and direction == "long":
                                if rsi_15m is None or rsi_15m < SPIKE_MIN_RSI:
                                    continue
...
def main():
        ...
        try:
            ...
            for trade in list(open_positions):
                if trade["symbol"] not in real_symbols:
                    ...
                    trade["status"] = "closed"
                    # A/B双跑: 同步平仓也释放锁
                    release_symbol_lock(trade["symbol"], STRATEGY_PROFILE)
    ...
    # A/B双跑: 加入其他profile已锁定的symbol
    locked_by_others = get_locked_symbols(exclude_profile=STRATEGY_PROFILE)
    open_symbols |= locked_by_others
...
# 开仓参数
        ...
        # A/B双跑: 开仓前获取symbol锁
        if not acquire_symbol_lock(verified["symbol"], STRATEGY_PROFILE):
            log(f"  {verified['symbol']} 已被其他Profile锁定，跳过")
            continue
