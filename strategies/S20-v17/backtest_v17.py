#!/usr/bin/env python3
"""
回测脚本 v17 — v6极端费率 + v14蓄势突破 合并版

v6(56笔/64%/+$688)是最强策略，核心是极端费率信号。
v14(42笔/52%/+$607)蓄势突破做空62%胜率。

合并策略:
- 重新启用extreme_neg_funding(v6主力:45笔/64%胜率/+$462)
- 保留extreme_pos_funding(v6:10笔/70%胜率/+$296)
- 保留coiling_breakout(v14:42笔/52%胜率/+$607)
- 保留pump_short
- 保留v14所有数据驱动优化(RSI≥65做多跳过/ATR≥2%禁止做多/做空仓位控制等)
"""
import sys
import json
import time
...
MAX_POSITIONS = 4             # v17: 3→4, 容纳极端费率+蓄势突破并行
...
def run_backtest():
    print("=" * 60)
    print(f"📊 回测 v17 — v6极端费率 + v14蓄势突破 合并版")
    ...
    print(f"v17合并: 极端负费率做多 + 极端正费率做空 + 蓄势突破 + v14数据驱动优化")
    print("=" * 60)
    ...
    for step_i, ts in enumerate(all_times):
        ...
        if (is_funding_time or should_scan_coiling) and len(positions) < MAX_POSITIONS and balance > 100:
            ...
            if is_funding_time:
                ...
                # v17: 重新启用极端负费率(v6主力45笔/64%胜率/+$462)
            ...
            for c in valid:
                ...
                tech = c.get("tech", {})
                is_funding_signal = c.get("type", "").startswith("extreme_")
                
                # v12b: 做多RSI限制 — RSI≥65跳过 (极端费率信号豁免,v6:45笔/64%胜率)
                if c["direction"] == "long" and tech.get("rsi", 50) >= 65 and not is_funding_signal:
                    signals_filtered += 1
                    continue
                
                # v14: ATR≥2%禁止做多 (极端费率信号豁免)
                if c["direction"] == "long" and tech.get("atr_pct", 0) >= 0.02 and not is_funding_signal:
                    signals_filtered += 1
                    continue
                
                # v17: 极端费率做多不减半(自身64%胜率已验证), 蓄势突破做多减半
                long_factor = 0.5 if (c["direction"] == "long" and not is_funding_signal) else 1.0
                ...
                # v14: 优化3 — env_score=5(BTC方向不匹配)仓位半价 (1W/4L净亏$150)
                ...
                # v14: 优化2 — 低ATR(<0.7%)做空仓位上限$700 (低波动做空易被反抽止损)
                if c["direction"] == "short" and tech.get("atr_pct", 999) < 0.007 and pos_usd > 700:
                    pos_usd = 700
                
                # v14b: 优化4 — 信号强度A且ATR≥1.4%做空仓位上限$400 (高波动A级做空假信号多)
                if (c["direction"] == "short" and c.get("strength", "") == "A"
                    and tech.get("atr_pct", 0) >= 0.014 and pos_usd > 400):
                    pos_usd = 400
    ...
    # ============================================
    print("\n" + "=" * 60)
    print(f"📊 回测 v17 结果 — v6极端费率 + v14蓄势突破 合并版")
    print("=" * 60)
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v17_result.json"
