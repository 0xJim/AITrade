...
def get_klines(symbol: str, interval: str, limit: int = 120) -> list[Candle]:
    raw = api_get("/fapi/v1/klines", {"symbol": symbol, "interval": interval, "limit": limit})
...
def scan_symbol(sym: str, args: argparse.Namespace, st: dict) -> dict | None:
    k15 = closed_candles(get_klines(sym, "15m", 160), MS_15M)
