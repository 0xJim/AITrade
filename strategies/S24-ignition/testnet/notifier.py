...
HERMES_API = os.environ.get("HERMES_API_URL", "http://localhost:8643")


def _hermes_send(text: str, target: str) -> bool:
    """通过 hermes CLI send 命令发送消息"""
    import subprocess
    try:
        ...
        print(f"[notifier] hermes send to {target} failed: {e}")
        return False


def send_wechat(text: str) -> bool:
    """通过 hermes CLI 发微信"""
    ok = _hermes_send(text, "weixin")
    if not ok:
        # fallback: 存本地
...
def send_telegram(text: str) -> bool:
    """通过 hermes CLI 发 Telegram"""
    return _hermes_send(text, "telegram")
