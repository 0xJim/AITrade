...
基于v14b回测数据(42笔/+$607/52.4%胜率)深度分析亏损模式后实施2项优化:
1. 做空A级+ATR≥1.4%跳过: 6笔净亏$202(仅WIFUSDT赚$90)
2. ATR<0.7%做空跳过: 2笔全亏$79(BNB+PAXG)
净预期提升: +$371, 目标+$978
"""
import sys
import json
...
def run_backtest():
    ...
    print(f"v15优化: ATR≥2%禁止做多 | 低ATR做空<$700 | env=5仓位半价 | 做空A级高ATR跳过 | ATR<0.7%做空跳过 | RSI做多<65 | 做多仓位0.5x | 做空优先")
    print("=" * 60)
    ...
    for step_i, ts in enumerate(all_times):
        ...
        if (is_funding_time or should_scan_coiling) and len(positions) < MAX_POSITIONS and balance > 100:
            ...
            for c in valid:
                ...
                # v15: 优化1 — ATR<0.7%做空直接跳过 (2笔全亏$79, BNB+PAXG都是低波做空被反抽)
                if c["direction"] == "short" and tech.get("atr_pct", 999) < 0.007:
                    signals_filtered += 1
                    continue
                
                # v15: 优化2 — 做空A级+ATR≥1.4%直接跳过 (6笔净亏$202, 仅WIFUSDT赚$90)
                if (c["direction"] == "short" and c.get("strength", "") == "A"
                    and tech.get("atr_pct", 0) >= 0.014):
                    signals_filtered += 1
                    continue
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v15_result.json"
