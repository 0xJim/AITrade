#!/usr/bin/env python3
"""
回测脚本 v12b — 基于v10c的3项优化版(调整版)

基于v10c回测数据深度分析后实施3项优化:
1. 做多RSI限制: RSI≥65跳过(仅过滤极端超买,修复v12误杀RSI55-64赚钱做多的问题)
2. 做多仓位系数0.5: 做多胜率41% vs 做空62%, 合理降低做多风险暴露
3. 信号类型排序: pump_short > extreme_pos_funding > coiling_breakout > crash_bounce

预期: 保留赚钱做多(WIF/XAG/TON/SNDK等 +$816)的同时减少亏损做多(-$579)
净提升约 +$237 vs RSI≥55的版本
"""
import sys
import json
import time
...
LEVERAGE = 3
RISK_PER_TRADE = 0.015       # v10: 2%→1.5% — 回到安全区
MAX_POSITIONS = 3             # 保持3
COOLDOWN_HOURS = 12           # 保持12h
COOLDOWN_CONSECUTIVE_LOSS = 96  # 保持96h(4天)
DAILY_MAX_LOSS_PCT = 4        # v10: 5→4% — 稍微收紧
...
# ============================================================
# v13 新增: 从v10融合的多时间框架分析 + 六维评分
# ============================================================

# MTF框架定义
MTF_FRAMES = ["30m", "1h", "4h", "6h", "8h", "12h", "1d", "3d", "1w", "1M"]
LARGE_FRAMES = {"1d", "3d", "1w", "1M"}
MTF_CONSISTENCY_WEIGHT = 0.30
MTF_LARGE_WEIGHT = 0.15
MTF_SMALL_WEIGHT = 0.10
MTF_MIN_AGREE = 3
...
def aggregate_klines_v13(klines_src, target_minutes):
    """从1h K线聚合更大时间框架 (v10移植)"""
    if not klines_src:
        return []
...
def mtf_trend_from_klines_v13(klines, direction):
    """单框架趋势判断 (v10移植)"""
...
def mtf_analyze_v13(klines_by_frame, direction):
    """多时间框架一致性分析 (v10移植)"""
    frame_trends = {}
    agree_count = 0
    large_agree = 0
    ...
    for frame, klines in klines_by_frame.items():
        if not klines or len(klines) < EMA_SLOW + 5:
            ...
            continue
        trend = mtf_trend_from_klines_v13(klines, direction)
...
def calc_fgi_from_btc_v13(klines, current_price):
    """从BTC K线模拟FGI (v10移植)"""
...
def v8_calc_weights_v13(signal, tech, btc_chg, fgi):
    """六维评分 — 完整做空支持 (v10移植)"""
...
def v8_signal_quality_v13(signal, tech, mtf_result):
    """信号质量评分 0-100 (v10移植)"""
    score = 50
    direction = signal["direction"]
    trend = tech["trend"]
...
def run_backtest():
    print("=" * 60)
    print(f"📊 回测 v13 — v10 MTF+六维评分 x v12b信号类型")
    ...
    print(f"止损: 低ATRx{ATR_SL_MULTIPLIER_LOW}/高ATRx{ATR_SL_MULTIPLIER_HIGH}({MIN_SL_PCT*100}-{MAX_SL_PCT*100}%) | RR>={MIN_RR_RATIO}")
    ...
    print(f"v13融合: MTF多框架+六维评分+信号质量 | RSI做多<65 | 做多仓位0.5x | 信号排序优先做空")
    print("=" * 60)
    ...
    # v13 新增: 预聚合多时间框架
    print("\n🔄 预聚合多时间框架...")
    frame_minutes = {"30m": 30, "1h": 60, "4h": 240, "6h": 360, "8h": 480,
    ...
    for sym, kl in all_klines.items():
        ...
        for frame, mins in frame_minutes.items():
            if frame == "1h":
                continue
            mtf_cache[sym][frame] = aggregate_klines_v13(kl, mins)
    print("  完成")
    ...
    for step_i, ts in enumerate(all_times):
        ...
        if (is_funding_time or should_scan_coiling) and len(positions) < MAX_POSITIONS and balance > 100:
            ...
            # v12: 信号类型按历史胜率优先级排序
            # 优先级: pump_short(71%) > extreme_pos_funding(62%) > coiling_breakout(54%) > crash_bounce(29%)
            # 做空优先于做多 (相同类型下做空优先)
            type_order = {"pump_short": 0, "extreme_pos_funding": 1, "coiling_breakout": 2}
            def sort_key(x):
                st = type_order.get(x.get("type", ""), 3)
                dir_order = 0 if x.get("direction") == "short" else 1
                strength_order = {"S": 0, "A": 1}.get(x.get("strength", ""), 2)
                return (st, dir_order, strength_order, -x.get("env_score", 0))
            valid.sort(key=sort_key)

            # v13 新增: 对每个候选信号计算MTF+六维评分+信号质量
            scored_candidates = []
            for c in valid:
                sym = c["symbol"]
                tech_c = c.get("tech", {})
                ...
                # MTF分析
                klines_by_frame = mtf_cache.get(sym, {})
                mtf_result = mtf_analyze_v13(klines_by_frame, c["direction"])

                # 获取最新费率信息
                fr_val = 0
                f_hist = all_funding.get(sym, [])
                if f_hist:
                    recent_fr_f = [f for f in f_hist if f["time"] <= ts]
                    if recent_fr_f:
                        fr_val = recent_fr_f[-1]["rate"] / 100
                ...
                v8_score_val, v8_scores = v8_calc_weights_v13(signal_for_v8, tech_c, btc_chg, 50)

                # 信号质量
                quality = v8_signal_quality_v13(signal_for_v8, tech_c, mtf_result)
                ...
                # 2. MTF至少2个框架同意, 或者六维评分方向明显(>15)
                if mtf_result["agree_count"] < 2 and abs(v8_score_val) < 15:
                    signals_filtered += 1
                    continue

                # 存储额外评分数据
                c["v8_score"] = round(v8_score_val, 2)
                c["quality"] = quality
                c["mtf_agree"] = mtf_result["agree_count"]
                c["mtf_total"] = mtf_result["total_frames"]
                c["mtf_consistency"] = mtf_result["consistency_score"]
                scored_candidates.append(c)

            # v13: 重新按质量+MTF排序
            scored_candidates.sort(key=lambda x: (
                0 if x.get("direction") == "short" else 1,  # 做空优先
                type_order.get(x.get("type", ""), 3),
                {'S': 0, 'A': 1}.get(x.get("strength", ""), 2),
                -x.get("quality", 0),  # 质量高的优先
                -x.get("mtf_agree", 0),  # MTF同意多的优先
            ))
            ...
            for c in scored_candidates:
                ...
                # v12b: 做多RSI限制 — RSI≥65跳过(仅过滤极端超买,保留RSI55-64的赚钱做多)
                tech = c.get("tech", {})
                if c["direction"] == "long" and tech.get("rsi", 50) >= 65:
                    signals_filtered += 1
                    continue
                
                # v12: 做多仓位减半(数据:做多胜率41% vs 做空62%)
                ...
                mtf_info = c.get("mtf_agree", 0)
                mtf_total = c.get("mtf_total", 0)
                v8score = c.get("v8_score", 0)
                print(f"  开仓 #{position['id']} {c['symbol']} {'多' if c['direction']=='long' else '空'} @{price:.4f} "
                      ...
                      f"质量={quality:.0f} MTF={mtf_info}/{mtf_total} v8={v8score:+.0f} "
    ...
    # ============================================
    print("\n" + "=" * 60)
    print(f"📊 回测 v13 结果 — v10 MTF+六维 x v12b信号类型")
    print("=" * 60)
    ...
    # v13 新增: MTF/六维评分/质量分段统计
    print(f"\n📊 v13新增维度统计:")
    if closed:
        # 按MTF一致数分
        mtf_buckets = defaultdict(lambda: {"win": 0, "loss": 0, "pnl": 0.0})
        for t in closed:
            mtf = t.get("mtf_agree", 0) or 0
            key = f"MTF≥{mtf}"
            if t.get("pnl_usd", 0) > 0: mtf_buckets[key]["win"] += 1
            else: mtf_buckets[key]["loss"] += 1
            mtf_buckets[key]["pnl"] += t.get("pnl_usd", 0)
        for key, s in sorted(mtf_buckets.items(), key=lambda x: -len(x[1])):
            total = s["win"] + s["loss"]
            wr = s["win"] / total * 100 if total > 0 else 0
            print(f"  {key}: {total}笔 胜{s['win']} 胜率{wr:.0f}% PnL {s['pnl']:+.1f}U")

        # 按质量分
        quality_buckets = {"<50": [], "50-59": [], "60-69": [], "70-79": [], "80+": []}
        for t in closed:
            q = t.get("v8_quality", 0) or 50
            if q < 50: quality_buckets["<50"].append(t)
            elif q < 60: quality_buckets["50-59"].append(t)
            elif q < 70: quality_buckets["60-69"].append(t)
            elif q < 80: quality_buckets["70-79"].append(t)
            else: quality_buckets["80+"].append(t)
        for k, ts in quality_buckets.items():
            if not ts: continue
            pnl = sum(t.get("pnl_usd", 0) for t in ts)
            w = sum(1 for t in ts if (t.get("pnl_usd", 0) or 0) > 0)
            print(f"  质量{k}: {len(ts)}笔 胜{w} PnL {pnl:+.1f}U")

    # v13 对比: v10c vs v12b vs v13
    print(f"\n📊 v10c vs v12b vs v13 对比:")
    print(f"  {'指标':<20} {'v10c':>12} {'v12b':>12} {'v13':>12}")
    print(f"  {'-'*56}")
    if closed:
        wr_v13 = f"{len(wins)/len(closed)*100:.0f}%"
        pnl_v13 = f"{total_pnl:+.0f}U"
        dd_v13 = f"-{max_drawdown:.0f}%"
    else:
        wr_v13 = pnl_v13 = dd_v13 = "N/A"
    print(f"  {'胜率':<20} {'56%':>12} {'51%':>12} {wr_v13:>12}")
    print(f"  {'总盈亏':<20} {'+$958':>12} {'+$590':>12} {pnl_v13:>12}")
    print(f"  {'最大回撤':<20} {'-7%':>12} {'-3.2%':>12} {dd_v13:>12}")

    # 保存
    result = {
        "version": "v13",
        "description": "v10 MTF+六维评分 x v12b信号类型",
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v13_result.json"
