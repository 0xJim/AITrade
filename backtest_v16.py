...
def run_backtest():
    ...
    for step_i, ts in enumerate(all_times):
        ...
        if (is_funding_time or should_scan_coiling) and len(positions) < MAX_POSITIONS and balance > 100:
            ...
            for c in valid:
                ...
                # v16: 做多RSI放宽至75
                tech = c.get("tech", {})
                if c["direction"] == "long" and tech.get("rsi", 50) >= 75:
                    signals_filtered += 1
                    continue
                
                # v16: 删除ATR≥2%禁做多(v15一刀切太严)
                # if c["direction"] == "long" and tech.get("atr_pct", 0) >= 0.02:
                #     signals_filtered += 1; continue
                
                # v16: 做多仓位0.8x(v15的0.5太严)
                long_factor = 0.8 if c["direction"] == "long" else 1.0
                ...
                # v16: 删除ATR<0.7%做空跳过(改为上面$700仓位限制)
                
                # v16: 删除做空A级+ATR≥1.4%跳过(6笔太少不具统计意义)
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v16_result.json"
