#!/usr/bin/env python3
"""
回测脚本 v14 — 基于v12b的3项数据驱动优化

基于v12b回测数据(39笔/+$590/51.3%胜率)深度分析亏损模式后实施3项优化:
1. ATR≥2%禁止做多: 3笔全亏损$98, ATR高时做多假突破多
2. 低ATR(<0.7%)做空仓位上限$700: BNBUSDT+PAXGUSDT亏损$121(仓位$2100)
3. env_score=5(BTC方向不匹配)仓位半价: 1W/4L净亏$150

预期: 保留v12b赚钱交易的同时减少3类可识别亏损, 净提升约+$200
"""
import sys
import json
import time
...
def run_backtest():
    print("=" * 60)
    print(f"📊 回测 v14 — 基于v12b的3项数据驱动优化")
    ...
    print(f"v14优化: ATR≥2%禁止做多 | 低ATR做空<$700 | env=5仓位半价 | 高ATR+A级做空<$400 | RSI做多<65 | 做多仓位0.5x | 做空优先")
    print("=" * 60)
    ...
    for step_i, ts in enumerate(all_times):
        ...
        if (is_funding_time or should_scan_coiling) and len(positions) < MAX_POSITIONS and balance > 100:
            ...
            for c in valid:
                ...
                # v14: 优化1 — ATR≥2%禁止做多 (ATR高时做多假突破多,历史3笔全亏$98)
    ...
    # ============================================
    print("\n" + "=" * 60)
    print(f"📊 回测 v14 结果 — 基于v12b的3项数据驱动优化")
    print("=" * 60)
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v14_result.json"
