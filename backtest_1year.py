...
- 持仓<12h: 0W/3L亏$246全被SL扫 → 入场后4h宽限期不扫SL
- 持仓>48h: 7W/1L胜率88%赚$478 → MAX_HOLD延长到72h
- WIFUSDT 5W/0L赚$480 → 明星币种确认趋势后可以重复做
v10b→v10c改动:
1. 入场后4h宽限期(grace period)，不检查SL
2. MAX_HOLD_HOURS: 48→72h (延长持仓让利润跑)
3. 回退SL区间限制和做多env门槛(数据证明无效)
"""
import sys
import json
import time
...
START_TIME = datetime(2025, 1, 1, 0, 0, tzinfo=TZ_UTC8)
...
def run_backtest():
    ...
    out_path = Path(__file__).parent / "data" / "backtest_v10c_result.json"
