...
- G60B = 低DD均衡档 (主推验证档)
- G60C = Clean Spike 小仓灰度档 (当前服务器模拟盘推荐)
- G60S = 低回撤严格档 (DD优先验证档)
...
MAX_OPEN_POSITIONS = int(get_config("MAX_OPEN_POSITIONS", "3"))  # 最大同时持仓
POSITION_PCT = float(get_config("POSITION_PCT", "10"))           # 每笔仓位占比% (Kelly动态调整)
LEVERAGE = 3                   # 杠杆
...
CLOSED_15M_ANOMALY_VOLUME_RATIO_MIN = float(get_config("CLOSED_15M_ANOMALY_VOLUME_RATIO_MIN", "1.8"))
CLOSED_15M_ANOMALY_BODY_RATIO_MIN = float(get_config("CLOSED_15M_ANOMALY_BODY_RATIO_MIN", "0.55"))
CLOSED_15M_ANOMALY_CLOSE_POSITION_MIN = float(get_config("CLOSED_15M_ANOMALY_CLOSE_POSITION_MIN", "0.65"))
...
V8_POSITION_PCT_MIN = float(get_config("V8_POSITION_PCT_MIN", "2"))
V8_POSITION_PCT_MAX = float(get_config("V8_POSITION_PCT_MAX", "20"))
...
# 动态小黑屋: 达到亏损标准后临时隔离，到期自动释放
BLACKLIST_LOOKBACK_DAYS = int(get_config("BLACKLIST_LOOKBACK_DAYS", "30"))
BLACKLIST_MIN_TRADES = int(get_config("BLACKLIST_MIN_TRADES", "3"))
BLACKLIST_MAX_LOSS_USD = float(get_config("BLACKLIST_MAX_LOSS_USD", "50"))
BLACKLIST_MAX_WIN_RATE = float(get_config("BLACKLIST_MAX_WIN_RATE", "0.40"))
BLACKLIST_SINGLE_LOSS_USD = float(get_config("BLACKLIST_SINGLE_LOSS_USD", "12"))
BLACKLIST_QUARANTINE_HOURS = int(get_config("BLACKLIST_QUARANTINE_HOURS", "72"))
BLACKLIST_SHORT_V8_SCORE_THRESHOLD = int(get_config("BLACKLIST_SHORT_V8_SCORE_THRESHOLD", "5"))
BLACKLIST_SHORT_POSITION_FACTOR = float(get_config("BLACKLIST_SHORT_POSITION_FACTOR", "0.5"))
...
STRATEGY_PROFILES = {
    ...
    # G60C: Clean Spike 小仓灰度档，当前服务器模拟盘推荐。
    # 用途: 先过滤 05-15~05-18 暴露的假 Spike 和无保护单风险，再观察 3~7 天。
    # 回测口径: strict Spike 15m 收线信号，下一根 15m open 入场，手续费/滑点计入。
    "G60C": {
        ...
        "V8_POSITION_PCT_MAX": 4.0,
        "desc": "Clean Spike 小仓灰度档(服务器模拟盘推荐)",
    },
...
if STRATEGY_PROFILE in STRATEGY_PROFILES:
    ...
    V11I_SHORT_V8_THRESHOLD = profile_config.get("V11I_SHORT_V8_THRESHOLD", V11I_SHORT_V8_THRESHOLD)
    V11I_SHORT_V8_MULT = profile_config.get("V11I_SHORT_V8_MULT", V11I_SHORT_V8_MULT)
...
ACTIVE_STATIC_BLACKLIST = sorted(
    set(STATIC_BLACKLIST) | set(PROFILE_EXTRA_BLACKLIST)
)
